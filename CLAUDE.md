...
- **LangChain** + **Ollama** (local) / **Google Gemini** (nuvem) — formatação/análise/condensação/descrição de imagens; **RAG local** via `OllamaEmbeddings` (`nomic-embed-custom`, CPU, 768-dim); busca **híbrida** (denso+**BM25** via `rank-bm25`, dependência base — puro Python/numpy, sem scipy — combinados por Reciprocal Rank Fusion)
