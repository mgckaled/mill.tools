...
- **scikit-learn** (extra opcional `[ml]` ≥1.4, Plano 3) — ML clássico **torch-free**; usado só pelos algoritmos dos Planos 4/5 (classificação/clustering/regressão). A **fundação** de ML (`core/ml/features.py` acessor + `dedup.py`) é **numpy-pura** e não exige o extra; `joblib` (persistência de modelos versionada) vem transitivo
...
│   ├── library/                — types.py (LibraryItem) · scanner.py · thumbnails.py · analytics.py (Plano 2)
...
│   └── ml/                      — types · features (acessor de embeddings, numpy-puro: pool do VectorStore) · dedup (cosseno) · deps (gate [ml]) · store (modelos versionados) (Plano 3)
└── gui/
...
- **CLI** (`cli/ai.py`): `ai index` · `ai stats` (resumo read-only, **+ timing por modelo** no Plano 2) · `ai dups` (duplicatas por similaridade, **read-only/sem embedder** — fundação de ML do Plano 3; `--threshold`/`--scope <kind>`) · `ai "pergunta"` · `--scope`/`--model`/`--k`/`--reindex`/`--batch [--kind]`. Embeddings sempre locais.
...
uv run main.py ai index | ai stats | ai dups [--threshold 0.95] [--scope kind] | ai "pergunta" [--scope X] [--model gemini-2.5-flash] [--k 8] [--batch]
