...
- **LangChain** + **Ollama** (local) / **Google Gemini** / **Zhipu GLM** (nuvem, ambos opt-in) — formatação/análise/condensação/descrição de imagens; GLM via `langchain-openai` (`ChatOpenAI` com `base_url` da Zhipu — API OpenAI-compatible, evita o `ChatZhipuAI` do `langchain_community`/`pyjwt`, legado); **RAG local** via `OllamaEmbeddings` (`nomic-embed-custom`, CPU, 768-dim); busca **híbrida** (denso+**BM25** via `rank-bm25`, dependência base — puro Python/numpy, sem scipy — combinados por Reciprocal Rank Fusion)
...
- **yake** + **spacy** (extra opcional `[nlp]`, Plano 4B) — NLP textual **torch-free**: keyphrases (YAKE, estatístico) e NER (spaCy CNN `pt_core_news_sm`, **nunca `_trf`** que puxaria torch). O resumo extractivo (`summarize.py`) é **self-contained** sobre o `TfidfVectorizer` do `[ml]` (sem dep nova, sem download nltk). Classificação (`classify.py`) é numpy + `[ml]` (sem dep nova) e **reusável por domínio** (perfil de transcrição/domínio de dados/tipo de documento, Tier A do `docs/plan/PLANO_ML_NOVAS_FEATURES.md`). O **modelo spaCy** é download à parte (como o Tesseract)
- **dHash** (`core/image/dhash.py`, zero dependência nova — só Pillow+numpy) — hash perceptual para dedup de imagens na Biblioteca; **IsolationForest** (já em `[ml]`) — outliers tabulares no módulo Dados
...
│   ├── image/                   — args.py + downloader/converter/info + transform/background/describe/exif/smart_crop/filter_previews/ocr + **dhash.py** (perceptual hash zero-dep, dedup de imagens)
│   ├── document/                — args.py + downloader/converter/info + processor/converter/info/ocr/qr
│   ├── library/                — types.py (LibraryItem, **ImageDuplicateGroup**) · scanner.py (filter_items aceita tag_index) · thumbnails.py · analytics.py (Plano 2) · tags.py (auto-tags por keyphrase, cache, Plano 4B) · **image_dedup.py** (near_duplicate_images, componentes conexas por Hamming, espelha ml/dedup.py)
│   ├── rag/                    — types · embedder (única rede) · store (VectorStore, cache de `_normalized` **e `_bm25`**) · indexer · retriever (busca **híbrida** BM25+denso via RRF) · **bm25.py** (build_bm25_index/bm25_score) · chat · templates · batch · stats · analytics (Plano 2)
│   ├── recipes/                — types · registry (registry/<módulo>.py) · runner · validate · inputs · presets · store · history (Plano 2)
│   ├── data/                   — types · scanner · engine (única fronteira DuckDB; preview/abas XLSX) · nl2sql · validate · convert · profile · assess (qualidade IA) · datacard (cartão indexável) · store · **ml.py** (detect_outliers via IsolationForest, fronteira `[ml]` sobre `frames.to_pandas`)
│   ├── ml/                      — types · features (acessor de embeddings, numpy-puro: pool do VectorStore) · dedup · recommend (related/in_corpus, numpy, reranking por **MMR**) · deps (gate [ml]/[ml-viz]) · store (modelos versionados) · cluster (HDBSCAN/k-means, **k-means aceita `k=None` → auto-seleção por `silhouette_score` acima de 20 documentos**) · labeling (c-TF-IDF, `ngram_range=(1,3)` + `reduce_frequent_words`) · project (PCA/**TSNE**/UMAP 2D) · cache (mapa versionado) · mapviz (SemanticMap→PNG, `build_semantic_map` aceita `on_stage` p/ o stepper do Observatório) · classify (zero-shot/supervisionado, **parametrizado por `domain`** — `DOMAIN_TRANSCRIPTION_PROFILE`/`DOMAIN_DATA`/`DOMAIN_DOCUMENT`, Tier A) (Planos 3/4A/4B; refinamentos em `docs/plan/PLANO_REFINAMENTO_ML_TEXTO_RAG.md`; novas features em `docs/plan/PLANO_ML_NOVAS_FEATURES.md`)
...
│   └── observatory/             — **novo** (Tier A): pacote puro cross-módulo, sem gate próprio. `activity.py` (log append-only `~/.mill-tools/ml_activity.json`, cap 200, mesmo padrão de `recipes/history.py`) · `status.py` (agrega gates/domínios do classify/config em vigor/timings — nada novo, só leitura; `config_snapshot` lê defaults reais via `inspect.signature`, nunca uma cópia hardcoded)
├── cli/                         — bus.py (CLIEventBus) + 1 módulo por subcomando (audio/video/image/document/library/ai/recipes/data/**observatory**) + transcription.py (helpers)
└── gui/
    ...
    ├── modules/                — base.py (Module) · _pipeline_runner.py · _charts.py (helper de gráfico dos painéis, Plano 2) · **_stepper.py** (chip de estágio ativo, reusável — Observatório) + 1 pasta/módulo (form_view/worker/view/pipeline_log; image+document têm blocks/; **observatory/** tem view/activity_tab/status_tab, sem worker — read-only)
...
- **6 ferramentas** (Áudio→Vídeo→Imagens→Transcrição→Documentos→Dados) na **NavigationRail**. **Biblioteca/IA/Receitas/Observatório** são **hubs** fora da rail (operam sobre as saídas de todos) — botões dourados no AppBar. Os 4 ainda estão em `MODULES`/`ft.Stack`; `_RAIL_MODULES` exclui os `_HUB_IDS`.
...
- **Entrada via Home Screen** (`home.py`): 6 ferramentas (grade 3 por linha = 3+3) + 4 hubs (cards largos, borda dourada, selo "HUB") sobre o moinho girando. Clique → `build_app(initial_module=id)`.
...
- **Dedup de imagens (Tier A)**: `core/library/image_dedup.py::near_duplicate_images` agrupa imagens perceptualmente idênticas por distância de Hamming entre **dHash** (`core/image/dhash.py`, zero dependência — só Pillow+numpy; deliberadamente não o pacote `imagehash`, que puxaria scipy/PyWavelets pelo `phash`). CLI `library dedup-images [--max-distance 8]`; GUI deferida (mesmo padrão "acessor pronto, GUI depois" do Plano 3).

## Módulo IA (PR7)

RAG local sobre o corpus: indexa o texto que você produziu, recupera os trechos relevantes e responde **citando as fontes**. Embeddings **100% locais** (Ollama); Gemini/GLM só opt-in na resposta. Torch-free (só `numpy`). Reusa `make_llm`/`split_text`/`EventBus`/`scan_library`.

- **Core puro** (`core/rag/`): `embedder.py` é a **única rede** (injetável como `embed_fn`; o resto é unit-testável sem Ollama). `VectorStore` = matriz numpy `(N,D)` com busca **híbrida** (cosseno + BM25 via `core/rag/bm25.py`, combinados por **Reciprocal Rank Fusion** em `retriever.retrieve()`; `rank-bm25` é dependência base — puro Python/numpy, escolhido sobre o `bm25s` mais rápido porque este puxa scipy para um ganho que só importa acima de ~1M documentos) + persistência `.npz`/`.json` em `~/.mill-tools/rag/`. O `.score` reportado por chunk continua o cosseno denso (não o valor fundido), preservando o contrato do aviso de fora-de-escopo. `build_index()` **incremental** por `(path, mtime)` (pula inalterados, reembeda alterados, reconcilia removidos); indexa kinds textuais (`transcription`/`document` + descrições `.txt`), tira o header de transcrição, chunka via `split_text` (1200/150). **PR9.3**: aceita `card_fn` injetável e inclui `kind="data"` — arquivos de dados são indexados pelo **cartão de dados** (`core/data/datacard.card_for_path`), nunca pelas linhas cruas (vide Módulo Dados). `chat.answer()` monta contexto numerado `[n]` sob prompt estrito; **o `[n]` é chaveado pelo documento distinto** (chunks do mesmo arquivo compartilham número), então as citações nunca passam do total de fontes (antes numerava por chunk e citava `[5]`/`[6]` com 4 badges). `templates.py` (prompt library) · `batch.py` (1 prompt sobre N docs).
- **GUI** (`modules/ai/`): hub no AppBar, split form|painel, auto-contido. `form_view` — escopo, modelo (`gemma3-4b-custom` default → `gemma3-1b-custom` → `qwen7b-custom` → `gemini-2.5-flash` → `glm-4.7-flash`), chips de prompt. `worker` — `run_ai_index`/`run_ai_answer` em thread daemon (`module_id="ai"`). `view` — resposta em `ft.Markdown` com fontes clicáveis (badge `[n]` amarra ao `[n]` da resposta); status calculado **fora da UI thread** (`is_available()` faz ping no Ollama).
...
- **Outliers tabulares (Tier A)**: `core/data/ml.py::detect_outliers` via `IsolationForest` (já em `[ml]`) sobre as colunas numéricas de `frames.to_pandas`; coluna `_anomaly_score` (menor = mais atípico), NaN imputado pela média (IsolationForest não aceita NaN), colunas não-numéricas ignoradas (encoding categórico é limitação conhecida, não implementada). CLI `data outliers <arquivo> [--contamination]`; passo de Receita `data.outliers`; GUI (botão na aba Consulta) deferida.

## Módulo Observatório (Tier A)

Hub read-only (sem worker/pipeline, como a Biblioteca) que centraliza **toda** a maquinaria de ML do app — não só o RAG. Promovido de "aba do hub IA" para hub próprio ao reconsiderar escopo: o `CLAUDE.md` já define hub como algo que "opera sobre as saídas de todos os módulos", e esta superfície cobre RAG/Biblioteca/Transcrição/Dados/Receitas — aninhá-la na IA seria descasamento semântico (quem quer saber por que o Dados sinalizou uma linha não pensaria em abrir "IA"). Zero dependência nova.

- **Core puro** (`core/observatory/`): `activity.py` — log append-only cross-módulo (`ActivityEntry(module, event, detail, timestamp)`) em `~/.mill-tools/ml_activity.json`, cap 200, mesmo padrão de `core/recipes/history.py`; escrito pelos workers/CLI runners no ponto natural de conclusão (RAG dedup/classify, Dados outliers, Biblioteca dedup de imagens), nunca pelas funções puras de `core/ml`/`core/text`. `status.py` — agrega o que já existe em outros módulos (gates de `[ml]`/`[ml-viz]`/`[nlp]`/embedder; contagem de rótulos + modelo supervisionado por domínio via `classify.domain_label_count`/`has_supervised_model`; `config_snapshot()` lê os parâmetros em vigor **direto das funções reais** via `inspect.signature` — nunca uma cópia hardcoded que poderia divergir; timings por modelo via `rag/analytics.model_timings`).
- **GUI** (`modules/observatory/`): hub auto-contido, sem `ProgressPanel`. Duas abas manuais (`visible=` num `Stack`, Flet 0.85 não tem `ft.Tabs`): **Atividade** (`activity_tab.py`, feed cronológico das últimas 15 entradas) e **Status** (`status_tab.py`, cartões por assunto — gates/classificador/config/timings). `on_mount` grava o timestamp mais recente do log em `last_ml_activity_seen` (zera o selo do botão) e recarrega a aba salva (`last_observatory_tab`).
- **Selo de novidades**: ver "Splash + Home Screen + branding" — botão do AppBar mostra a contagem de eventos não vistos.
- **Stepper contextual (infra pronta, integração nas 3 telas é fast-follow)**: `gui/modules/_stepper.py::build_stepper` — fileira de chips reusável que destaca o estágio ativo e marca os concluídos, alimentada só por eventos reais (a maioria das chamadas de ML termina em bem menos de 1s — nunca um tempo fabricado). `mapviz.build_semantic_map` já aceita `on_stage` (chamado antes de `cluster`→`project`→`label`; pulado em cache hit). Wiring nas 3 telas (Conversa da IA, Mapa da Biblioteca, Insights da Transcrição) requer uma ponte thread-safe (`page.pubsub`, não mutação direta de controle) para o callback disparado de dentro de `asyncio.to_thread` — deixado para uma passagem futura dedicada.
- **CLI** (`cli/observatory.py`): `observatory status` (gates/classificador/config/timings) · `observatory activity [--limit 15]` (feed). Read-only, sem `CLIEventBus`, mesmo padrão de `library`/`ai stats`.
- Ver `docs/plan/PLANO_ML_NOVAS_FEATURES.md` (item 3.5) para o histórico da decisão (Painel da IA → hub próprio) e o resto do Tier A (busca híbrida, outliers, dedup de imagens, classify por domínio).
...
- **Home** (`home.py`): 6 ferramentas (grade 3 por linha, `_tool_row` paga as sobras com spacers) + 4 hubs sobre o moinho girando. Cards **crescer-no-hover**: cada card é **um único `GestureDetector`** (tap + `on_enter`/`on_exit`) sobre um `Container` animado (sem `ink=True`); cresce no hover e revela o detalhe; reflow sem fixar altura das `Row` (só um card hovered por vez → cabe sem scroll). **Crítico**: `Container.on_hover` não dispara quando coberto — usar o `on_enter`/`on_exit` do próprio GD (ver tabela de quirks).
- **AppBar** (`app.py`): wordmark + botões-hub Biblioteca/IA/Receitas/Observatório (dourados quando ativos); Home/Splash/tema em `actions` (bloqueados se pipeline rodando). `page.pubsub.unsubscribe_all()` no início de `build_app` evita acúmulo de subscribers. **Selo de novidades do Observatório**: `observatory_btn.text` vira `"Observatório (N)"` quando há N entradas do log mais recentes que `last_ml_activity_seen` (config.json) — snapshot recomputado no build e em `navigate_to("observatory")` (que também zera o contador via `on_mount`), não empurrado ao vivo.
...
uv run main.py ai "pergunta" [--scope X] [--model gemini-2.5-flash] [--k 8] [--batch]   # busca híbrida BM25+denso (RRF)
...
uv run main.py observatory status | observatory activity [--limit 15]
```
...
- **Chunking** (`llm_utils.split_text`): formatter usa separadores por frase; analyzer/prompter os padrão. **Bypass de contexto longo** (`bypass_long_context=True` em analyzer/prompter): provedores em nuvem pulam chunking sempre via `llm_factory.is_cloud_model` (Gemini 1M, GLM 200K tokens); locais conhecidos pulam **até um teto de chars** — `llm_factory.LONG_CONTEXT_LOCAL_BUDGETS` (`gemma3-4b-custom`: 12000 chars ≈ 3K tokens), bem abaixo do `num_ctx`. Acima do teto, volta a fatiar.
...
- **Formatter** (`phi4mini-custom`, 4500/150) · **Analyzer** (`gemma3-4b-custom` local / GUI default `gemini-2.5-flash`; 4500/300, **perfil-dirigido** via `src/analysis/`) · **Prompter** (`gemma3-4b-custom`; 4500/200, ~40% compressão, remove CTAs). Ambos aceitam `glm-4.7-flash` como alternativa de nuvem ao Gemini.
...
| Análise / Prompt-ready | `gemini-2.5-flash`, `glm-4.7-flash`, `gemma3-4b-custom`, `qwen7b-custom` |
...
- **Refinamento ML/texto/RAG** ✅ — Tiers 1–3: correção de recall + cache na busca do RAG (Tier 1); MMR (`recommend`/`summarize`), c-TF-IDF com `ngram_range`, parâmetros afinados do YAKE, TextRank com viés de posição, glossário opcional do `EntityRuler` (Tier 2); TSNE como 3º método de projeção, auto-k via `silhouette_score` (Tier 3). Zero dependência nova. Ver `docs/plan/PLANO_REFINAMENTO_ML_TEXTO_RAG.md`.
- **Novas features de ML — Tier A** ✅ — busca **híbrida** RAG (BM25+RRF, `rank-bm25` base); outliers tabulares (`IsolationForest`); dedup de imagens (**dHash** hand-rolled, zero dep — corrigido de `imagehash`/scipy); `classify.py` **parametrizado por domínio** (transcrição/dados/documentos, integração de superfície adiada); **novo hub Observatório** (`core/observatory/` + `gui/modules/observatory/` + CLI `observatory`) centralizando atividade e status de ML cross-módulo; stepper contextual (infra pronta, wiring nas 3 telas adiado). Ver `docs/plan/PLANO_ML_NOVAS_FEATURES.md`.
