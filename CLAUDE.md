...
- **ETA (PR7.2.5)**: durante a transcrição, um rótulo abaixo da última linha transcrita mostra o tempo restante + fator de velocidade (`"≈ 3m 00s restantes · 0,85× tempo-real"`). `transcription/pipeline_log.format_eta(elapsed, end, audio_duration)` (puro, testado) usa média móvel (`elapsed` cobriu `end`s de `audio_duration`s); só aparece após **5%** transcrito (o início é ruidoso). `progress_view.py` captura `t0` em `transcribe_started`, atualiza por `transcribe_segment` e esconde ao sair da fase / nos estados terminais.
...
- **Core puro** (`src/core/rag/`): `embedder.py` é a **única rede** (isolada, injetável como `embed_fn`); o resto opera sobre callables e é unit-testável sem Ollama. `VectorStore` é uma matriz numpy `(N, D)` com busca cosseno + persistência `.npz`/`.json` em `~/.mill-tools/rag/` (serialização via `dataclasses.asdict` — slots não têm `__dict__`). `build_index()` é **incremental** por `(path, mtime)`: pula inalterados, reembeda alterados, reconcilia removidos; indexa as **kinds textuais** (`transcription`/`document` + descrições de imagem `.txt`), tira o cabeçalho de transcrição e chunka via `split_text` (1200/150). `chat.answer()` monta contexto numerado `[n]` sob prompt estrito ("responda só pelo contexto + cite; senão diga que não achou"). O `[n]` é chaveado pelo **documento distinto** (chunks do mesmo arquivo compartilham um número), então os marcadores da resposta nunca passam do total de fontes exibidas — antes numerava por chunk e citava `[5]`/`[6]` com só 4 badges. `templates.py` = prompt library (defaults + `~/.mill-tools/prompts.json`); `batch.py` aplica um prompt a N documentos.
...
- **Persistência**: `last_ai_model`, `last_ai_scope`, `last_embed_model`, `last_ai_tab` (conversa|indice) e `ai_answer_times` (janela móvel de durações por modelo, p/ a estimativa de tempo) em `config.json`; índice em `~/.mill-tools/rag/`; prompt library em `~/.mill-tools/prompts.json`.
- **CLI** (`src/cli/ai.py`): `ai index` (re)indexa; `ai stats` resume o índice (read-only); `ai "pergunta"` responde; `--scope <path|kind>` restringe; `--model`/`--k`; `--reindex`; `--batch [--kind]` aplica a pergunta como instrução a cada documento. Reaproveita o mesmo core; embeddings sempre locais.
...
- **`stats.py` (core puro)**: `index_stats(directory) → IndexStats` descreve o índice (docs, chunks, dim, modelo de embedding, tamanho em disco, atualizado em, `per_doc` ordenado por #chunks); `fmt_status_line()` → `"28 docs · 4.654 chunks · 20 jun 20:45"` (milhar com ponto, mês PT-BR manual sem `locale`); `fmt_disk_size()`, `fmt_thousands()`, `fmt_datetime()` e `chunks_for()` (pares `(idx, text)` p/ o drill-down). `VectorStore.persist()` grava um sidecar `index_info.json` (`embed_model`, `dim`, `created_at`); índices antigos sem sidecar → `embed_model="?"`.
- **Aba "Índice" no módulo IA** (`index_tab.py`): toggle de abas manuais `Conversa | Índice` (Flet 0.85 sem `ft.Tabs` → `visible=` num `Stack`). Cabeçalho global (`summary_card`) + tabela por documento (`ícone · nome · tipo · #chunks · caracteres · data`) num `Column` paginado (`_PAGE_SIZE=120` + "Carregar mais") + drill-down de chunks num `AlertDialog` (lê `meta.json` via `chunks_for` fora da UI thread). `view.py::_refresh_status` passa o `IndexStats` já computado a `index_tab.apply` (sincroniza ao entrar na aba e após reindex); persiste `last_ai_tab`. A linha de status do painel Conversa usa `fmt_status_line`.
- **Botão "Indexar no RAG"** (`index_button.py`): `rag_index_button(page)` aparece nos painéis de resultado dos produtores (Transcrição em `result_view.py`, Documentos→`analyze`/`extract`/`ocr`, Receitas com saída `.txt`/`.md`) e dispara `run_ai_index` (incremental) numa thread com um bus de captura privado — **não** usa pubsub, então não interfere nos eventos do módulo hospedeiro. Feedback inline + SnackBar; desabilita com `SETUP_HINT` quando o embedder está indisponível (gate fora da UI thread). Indexação **por escolha**, nunca automática.
- **Estimativa de tempo da resposta** (`timing.py`, puro/testado): a resposta é um `invoke()` bloqueante de comprimento desconhecido — **não há countdown honesto** (ao contrário da transcrição, que tem fração `end/audio_duration`). Em vez disso: cronômetro ao vivo + um "tempo típico" da **média móvel** das últimas 5 respostas daquele modelo (`record_duration`/`average`/`format_typical`/`compose_status`). `worker.py` emite `model_name`+`elapsed` no `answer_done`; `view.py` mostra `"Gerando resposta… 0:14 · ~28s (típico do …)"` e grava a duração em `ai_answer_times`. **O ticker roda no event loop da UI via `page.run_task` (async)** — um `control.update()` de thread daemon não repinta até o próximo `page.update()` da UI thread (mesmo padrão de `home.py`/`library.py`). O `k` do RAG é fixo em 6 na GUI (CLI tem `--k`).
...
uv run main.py transcribe <URL> --analyze --profile lecture  # perfil (default/lecture/interview/tutorial/scientific/administrative/literary/review/storytelling/notes/tldr/flashcards)
...
- **Analyzer** (`src/analyzer.py`): 4500 chars/300 overlap, merge parcial. **Perfil-dirigido** (`src/analysis/`): `analyze(..., profile="default")` resolve o perfil e **gera** prompt de análise/merge + relatório a partir dos `fields` do perfil; a temperatura vem do perfil. Tradução automática PT-BR genérica. Modelo padrão local: `gemma3-4b-custom` (melhor síntese que o `qwen7b-custom` na CPU — validado em campo; CLI `--am` + `analyzer.DEFAULT_MODEL`). Default da GUI segue `gemini-2.5-flash`. `profile="default"` reproduz o esquema legado de 10 campos byte-a-byte (`_format_report` virou wrapper sobre `analysis.format_report(default)`).
...
- **Prompter** (`src/prompter.py`): 4500 chars/200 overlap, ~40% de compressão. Remove CTAs/patrocinadores. Modelo padrão local: `gemma3-4b-custom` (CLI `--pm` + `prompter.DEFAULT_PROMPT_MODEL`). Default da GUI segue `gemini-2.5-flash`.
...
| `control.update()` de thread daemon | **não repinta** até o próximo `page.update()` da UI thread — um cronômetro/ticker numa `threading.Thread` parece travado. Para atualização periódica viva, rodar no event loop da UI via `page.run_task` (corotina async com `await asyncio.sleep`) — ver `ai/view.py` (cronômetro de resposta), `home.py`, `library.py` |
...
**Transcrição (stage específico):** `metadata_start/done`, `audio_cached`, `download_start/done`, `whisper_loading/loaded`, `transcribe_started`, `language_detected` (`audio_duration`), `vad_filtered` (`duration`, `duration_after_vad`, `removed` — silêncio pulado pelo VAD; logado como `[i] VAD removed Xs of silence (Y%)`), `transcribe_segment` (`end`, `is_low_confidence`), `transcribe_summary`, `format_*`, `analyze_*`, `translation_*`, `prompt_*`.
...
- **PR7.2** ✅ — Inspetor de índice (RAG) + indexação por escolha + ETA da Transcrição. Sem dependência nova, torch-free, reaproveita o core `src/core/rag/`. Faseado PR7.2.0→7.2.5, um commit por fase: (0) core `index_stats()` + sidecar `index_info.json`; (1) CLI `ai stats`; (2) linha de status com data completa; (3) aba "Índice" no módulo IA (inspetor: cabeçalho global + tabela paginada por documento + drill-down de chunks); (4) botão "Indexar no RAG" nos produtores (Transcrição/Documentos/Receitas), indexação incremental por escolha; (5) ETA na Transcrição (`format_eta`, média móvel gated a 5%). Ver `docs/ROADMAP_PR7.2_IA_INDICE.md`.
