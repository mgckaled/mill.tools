...
│   ├── ml/                      — types · features (acessor de embeddings, numpy-puro: pool do VectorStore) · dedup · recommend (related/in_corpus, numpy) · deps (gate [ml]/[ml-viz]) · store (modelos versionados) · cluster (HDBSCAN/k-means) · labeling (c-TF-IDF) · project (PCA/UMAP 2D) · cache (mapa versionado) · mapviz (SemanticMap→PNG) · classify (perfil zero-shot/supervisionado, Plano 4B) (Planos 3/4A/4B)
