...
STEP_REGISTRY: dict[str, StepSpec] = {
    ...
    # data (PR9). data.query is multi-input (consumes the whole data-file list,
    # like document.merge) and is declared as producing KIND_TEXT so the result
    # can feed text-consuming steps (closes the chain data.query → ai.answer).
    "data.query": StepSpec(
