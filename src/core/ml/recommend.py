...
#
# Recalibrated 08/07/2026 (PLANO_RAG_ESPACO_EMBEDDING, Fase 5) against a
# *genuine* reindex under the new scheme (task prefixes + contextual header +
# PDF cleanup) — an earlier attempt silently no-op'd (build_index's
# (path, mtime) incremental check never re-triggers on a scheme-only change;
# see indexer.build_index's `force` param, added to fix that) and produced
# invalid numbers that were corrected here once the real reindex ran. Method:
# best dense cosine (VectorStore.search top-1, same path as in_corpus()) for
# 10 questions clearly covered by a real personal corpus (Dune novels, tech/AI
# video transcripts, Claude's Constitution) vs. 5 questions clearly outside it
# (cooking, car maintenance, chess, taxes, dog training). In-corpus range:
# 0.7356-0.8684 (mean 0.794). Out-of-corpus range: 0.6540-0.7115 (mean 0.682)
# — the two bands are **cleanly separated** on this sample (gap ~0.024, unlike
# the invalid pre-fix measurement, which showed a false overlap because the
# "after" side was actually still the old embedding space).
#
# Chose 0.72 — inside the observed gap, closer to the out-of-corpus edge than
# to the in-corpus one (a slightly larger margin below the in-corpus minimum
# than above the out-of-corpus maximum), favoring zero false "out of scope"
# warnings on real coverage (the costlier error for a personal RAG tool) —
# harder real questions than this hand-picked sample may still score lower
# than 0.7356. Revisit if false negatives (missed genuinely out-of-scope
# questions) turn out to be the more common complaint in practice.
