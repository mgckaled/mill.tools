...
"read-only" spirit as the Library hub. Two manual tabs (Atividade | Status) —
Flet 0.85 has no ``ft.Tabs``.
"""
...
def build_observatory_module(
    ...
    body_stack = ft.Stack([activity_view, status_view], expand=True)

    control = ft.Column(
        controls=[
            ft.Row([tab_activity, tab_status], spacing=Space.xs),
            hairline(),
    ...
    def _on_mount(_payload: dict) -> None:
        ...
        # navigate_to right after this runs) and re-scan both tabs.
