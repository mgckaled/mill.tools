...
def _run_qr(args: DocumentArgs, emit: Callable) -> bool:
    ...
    t0 = time()
    emit("document_op_start", payload={
        "operation": "qr",
        "item_name": args.qr_data[:40] or "QR code",
        "item_idx": 1, "total": 1, "page_count": 0,
    })
    emit("log", payload={"message": pipeline_log.fmt_qr_detail(
        args.qr_data[:40], args.qr_size,
    )})
    out = generate_qr(args.qr_data, DOCUMENT_PROCESSED_DIR, size=args.qr_size, fmt=args.qr_fmt)
    elapsed = f"{time() - t0:.1f}s"
    size_kb = out.stat().st_size // 1024
    emit("document_op_done", payload={
        "operation": "qr",
        "output_path": str(out),
        "elapsed": elapsed,
        "item_idx": 1, "total": 1,
...
def _run_merge(args: DocumentArgs, emit: Callable) -> bool:
    ...
    for p in paths:
        try:
            ...
            emit("log", payload={"message": f"[*] Lendo {p.name} · {info.page_count} pág."})
        except Exception:
            pass
    emit("document_op_start", payload={
        "operation": "merge",
        "item_name": f"{len(paths)} arquivos",
        "item_idx": 1, "total": 1, "page_count": total_pages,
    })
    emit("log", payload={"message": pipeline_log.fmt_merge_detail(len(paths), total_pages)})
...
def _run_split(args: DocumentArgs, emit: Callable) -> bool:
    ...
    emit("document_op_start", payload={
        "operation": "split",
        "item_name": path.name,
        "item_idx": 1, "total": 1, "page_count": info.page_count,
    })
    emit("log", payload={"message": pipeline_log.fmt_split_detail(
        path.name, info.page_count, spec,
    )})
    ...
    emit("task_done", payload={"output_paths": [str(p) for p in parts], "failed_count": 0})
    return True
...
def _run_pdf_to_images(
    ...
    imgs = pdf_to_images(
        path, DOCUMENT_PROCESSED_DIR,
        fmt=args.image_fmt, dpi=args.dpi,
    ...
    emit("task_done", payload={"output_paths": [str(p) for p in imgs], "failed_count": 0})
    return True
...
def _run_images_to_pdf(args: DocumentArgs, emit: Callable) -> bool:
    ...
    emit("document_op_start", payload={
        ...
        "item_idx": 1, "total": 1, "page_count": total,
    })
...
def _run_extract(args: DocumentArgs, emit: Callable) -> bool:
    ...
    emit("log", payload={"message": pipeline_log.fmt_extract_detail(path.name, info.page_count)})
    txt_path, word_count = extract_text(path, DOCUMENT_PROCESSED_DIR)
    elapsed = f"{time() - t0:.1f}s"
    emit("log", payload={"message": f"[»] {info.page_count} páginas · ~{word_count} palavras"})
    if word_count >= 50:
        emit("log", payload={"message": '[i] Clique em "Analisar com IA" para processar o conteúdo'})
    emit("document_op_done", payload={
        "operation": "extract",
...
def _run_analyze(
    ...
    emit("document_op_done", payload={
        ...
        "item_idx": 1, "total": 1,
        "src_size_bytes": path.stat().st_size,
        "out_size_bytes": analysis_path.stat().st_size if analysis_path.exists() else 0,
        "extra_stats": {},
    })
    ...
    match op:
        case "compress":
            emit("log", payload={"message": pipeline_log.fmt_compress_detail(
                path.name, info.page_count, src_mb, args.image_quality,
            )})
            out = processor.compress_pdf(path, DOCUMENT_PROCESSED_DIR, args.image_quality)
        case "rotate":
            pages_desc = "todas as páginas" if args.rotate_pages == "all" else f"páginas {args.rotate_pages}"
            emit("log", payload={"message": pipeline_log.fmt_rotate_detail(
                path.name, info.page_count, pages_desc, args.angle,
            )})
            out = processor.rotate_pdf(path, DOCUMENT_PROCESSED_DIR, args.angle, args.rotate_pages)
        case "watermark":
            pct = int(args.watermark_opacity * 100)
            emit("log", payload={"message": pipeline_log.fmt_watermark_detail(
                path.name, info.page_count, args.watermark_text, pct,
            )})
            out = processor.watermark_pdf(
                path, DOCUMENT_PROCESSED_DIR,
                args.watermark_text, args.watermark_opacity, args.watermark_position,
            )
        case "stamp":
            emit("log", payload={"message": pipeline_log.fmt_stamp_detail(
                path.name, info.page_count, args.stamp_text,
            )})
            out = processor.stamp_pdf(path, DOCUMENT_PROCESSED_DIR, args.stamp_text)
        case "encrypt":
            emit("log", payload={"message": pipeline_log.fmt_encrypt_detail(
                path.name, info.page_count, src_mb,
            )})
