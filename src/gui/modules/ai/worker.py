"""Worker for the AI module's Conversa (answer) flow, in a background thread.

No Flet dependency — emits everything through the EventBus (module_id="ai"), so
it is unit-testable with a fake bus. The core (retrieve/answer) is imported
lazily to keep app startup light.

The reindex flow used to live here too; it moved to
``gui/modules/observatory/index_worker.py`` (Fase 0b, PLANO_NL2CLI_HUB_IA.md) —
the AI hub now only runs the Conversa.
"""
