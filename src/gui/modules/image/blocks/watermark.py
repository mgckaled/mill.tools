...
from src.gui.theme.components import Cursor, help_icon_for, section_label, segmented_selector
...
def build_watermark_block(page: ft.Page) -> tuple[ft.Column, WatermarkRefs]:
    ...
    size_col, size_slider = labeled_slider(
        label="Tamanho", value=40.0, min=8.0, max=120.0, divisions=112,
        fmt=lambda v: f"{int(v)}",
    )
    opacity_col, opacity_slider = labeled_slider(
        label="Opacidade", value=0.5, min=0.0, max=1.0, divisions=20,
    ...
    text_col = ft.Column([
        ...
        text_tf,
        ft.Row(
            [section_label("Cor"), text_color_tf], spacing=8,
    ...
    block = ft.Column(
        ...
        controls=[
            ft.Row(
                [section_label("Marca d'água"), ft.Container(expand=True), help_icon_for("image.watermark", page)],
                vertical_alignment=ft.CrossAxisAlignment.CENTER,
            ),
            section_label("Modo"),
            mode_grid,
    ...
    refs = WatermarkRefs(
        ...
        get_text=lambda: (text_tf.value or "").strip(),
