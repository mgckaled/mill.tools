"""Consulta tab — translate/run a query, preview the result, shape and save.

Owns the "entendi assim" review card (editable SQL), the paginated in-memory
result table and the return-shaping footer (rename columns, format, Save /
Conversar sobre / Salvar como Receita / Indexar no RAG). It also drives the
form's preview/run buttons and the live translation timer.
"""
...
from src.gui.modules.data.worker import start_query, start_save, start_translate
...
@dataclass
class QueryTab:
    """Handles the central router/view needs from the Consulta tab."""

    view: ft.Control
    on_preview: Callable[[], None]  # form "Pré-visualizar" button
    on_run: Callable[[], None]  # form "Executar" button
    on_sql_ready: Callable[[dict], None]  # data_sql_ready event
    on_result: Callable[[dict], None]  # data_result event
    on_saved: Callable[[dict], None]  # data_saved event
    on_error: Callable[[dict], None]  # task_error (translate/query/save flows)
    on_show: Callable[[], None]  # called when the tab becomes active


def build_query_tab(ctx: DataViewContext) -> QueryTab:
    """Build the Consulta tab and return its handles."""
    page = ctx.page
    form = ctx.form

    # Result/query state shared across this tab's handlers.
    ...
    async def _tick(typical: str | None) -> None:
        while not _ticker_stop.is_set():
            ...
            label = _ACTION_LABEL.get(ctx.action[0], "Processando")
    ...
    def _on_preview() -> None:
        ...
        if not form.get_files():
            ctx.toast("Selecione ao menos um arquivo.")
            return
        if form.get_mode() == "sql":
            # SQL mode: no translation — just surface the SQL in the review card.
            sql = form.get_text()
            if not sql:
                ctx.toast("Escreva uma consulta SQL.")
                return
            _show_review(sql, "")
            return
        question = form.get_text()
        if not question:
            ctx.toast("Escreva uma pergunta.")
            return
        ctx.action[0] = "translate"
        _begin()
        start_translate(
            ctx.bus, form.get_files(), question, model_name=form.get_model()
        )
    ...
    def _on_run() -> None:
        ...
        sql = _current_sql()
        if not sql:
            ctx.toast("Nada para executar — pré-visualize ou escreva a consulta.")
            return
        ctx.action[0] = "query"
    ...
    def _on_refine_with_ai(_e=None) -> None:
        ...
        if ctx.pipeline_running[0] or not form.get_files() or not _last_error[0]:
            return
        augmented = build_refine_prompt(
            form.get_text(), _last_failed_sql[0], _last_error[0]
        )
        ctx.action[0] = "translate"
        _begin()
        start_translate(
            ctx.bus, form.get_files(), augmented, model_name=form.get_model()
        )
    ...
    # ------------------------------------------------------------------
    # Result preview table (paginated, in-memory)
    # ------------------------------------------------------------------
    ...
    def _render_table() -> None:
        ...
        w = page_window(total, _page_idx[0], _PAGE_SIZE)
        page_rows = _result_rows[w.start : w.end]
        ...
        page_label.value = w.label
        prev_btn.disabled = not w.has_prev
        next_btn.disabled = not w.has_next
    ...
    def _show_result() -> None:
        ...
        _has_result[0] = True
        footer.visible = ctx.tab[0] == "consulta"  # actions belong to the query tab
        _page_idx[0] = 0
        _render_table()
    ...
    def _on_converse(_e=None) -> None:
        ...
        if _last_saved[0] and _last_saved[0].exists():
            ctx.nav[0]("ai", {"file": str(_last_saved[0])})
            return
        ctx.action[0] = "converse"
        _begin()
        # JSON is the most chat-friendly export for a tabular answer.
        start_save(ctx.bus, form.get_files(), _effective_sql(), "json", _save_stem())
    ...
    def _on_save_recipe(_e=None) -> None:
        ...
        try:
            save_recipe(recipe)
            ctx.toast(f'Receita "{name}" salva.', error=False)
        except Exception as exc:
            logging.getLogger(__name__).warning("[!] save recipe failed: %s", exc)
            ctx.toast("Não foi possível salvar a receita.")
    ...
    def on_saved(p: dict) -> None:
        _end()
        out = Path(p.get("output_path", ""))
        _last_saved[0] = out
        saved_row.visible = True
        saved_path_text.value = out.name
        if ctx.action[0] == "converse" and out.exists():
            ctx.nav[0]("ai", {"file": str(out)})
            return
        ctx.toast(f"Salvo em output/data/{out.name}", error=False)

    def on_error(p: dict) -> None:
        message = p.get("message", "Erro.")
        # A failed translate has no SQL; a failed query keeps the SQL it tried,
        # so the IA refinement can reference both.
        if ctx.action[0] == "translate":
    ...
    def on_show() -> None:
        footer.visible = ctx.tab[0] == "consulta" and _has_result[0]

    # ------------------------------------------------------------------
    # Controls — shared progress/error
    # ------------------------------------------------------------------
    ...
    pager = ft.Row(
        [result_status_text, ft.Container(expand=True), prev_btn, page_label, next_btn],
        vertical_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    fmt_dd = ft.Dropdown(
        label="Formato",
        value=settings.load().get("last_data_format", "csv"),
