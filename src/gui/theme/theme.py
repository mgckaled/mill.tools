...
def _color_scheme(dark: bool) -> ft.ColorScheme:
    ...
    return ft.ColorScheme(
        ...
        # superfícies — mapeamento M3 (background removido nesta versão do Flet)
        surface=c.bg,                       # fundo da janela / Scaffold
        on_surface=c.text,
        on_surface_variant=c.text_secondary,
        surface_container_low=c.surface,    # painéis, cartões
        surface_container=c.surface_variant, # chips, inputs, superfícies elevadas
        # contornos
        outline=c.outline,
...
def apply_theme(page: ft.Page) -> None:
    """Aplica temas claro e escuro à página sem alterar theme_mode."""
