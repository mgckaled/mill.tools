...
def test_format_report_with_full_analysis():
    ...
    out = _format_report(
        ...
        video_meta={"title": "X", "channel": "Y", "duration": "01:00:00", "url": "https://yt/x"},
    )
    assert "# X" in out
...
def test_format_report_no_actions_shows_placeholder():
    from src.analyzer import _format_report
    out = _format_report({"summary": "s", "action_items": []}, source_path=Path("t.txt"))
...
def test_format_report_meta_without_channel_or_duration_no_pipe_line():
    from src.analyzer import _format_report
    out = _format_report({"summary": "S"}, source_path=Path("t.txt"), video_meta={"url": "https://yt/x"})
...
def test_ensure_portuguese_emits_language_detected_event():
    ...
    llm = _fake_llm("pt")
    _ensure_portuguese(_VALID_ANALYSIS, llm, on_event=lambda t, s, p: events.append((t, s, p)))
...
def test_analyze_single_chunk_writes_report(tmp_path, mocker, isolate_analysis_dir):
    ...
    mocker.patch.object(
        analyzer, "make_llm",
        side_effect=[
            _fake_llm(json.dumps(_VALID_ANALYSIS, ensure_ascii=False)),  # llm (analysis)
...
def test_analyze_single_chunk_with_transcription_appends_section(tmp_path, mocker, isolate_analysis_dir):
    from src import analyzer
    src = tmp_path / "video.txt"
    src.write_text(_HEADER + "\n\nshort body.", encoding="utf-8")
    mocker.patch.object(
...
def test_analyze_multi_chunk_calls_merge(tmp_path, mocker, isolate_analysis_dir):
    ...
    mocker.patch.object(
        analyzer, "make_llm",
        side_effect=[
            _fake_llm(*([json.dumps(partial, ensure_ascii=False)] * len(chunks)),
