...
def test_build_prompt_ready_empty_body_returns_input_path(tmp_path, mocker, isolate_digest_dir):
...
def test_build_prompt_ready_single_chunk_writes_output(tmp_path, mocker, isolate_digest_dir):
    from src import prompter
...
def test_build_prompt_ready_single_chunk_skips_merge(tmp_path, mocker, isolate_digest_dir):
    """When only 1 chunk, MERGE_CONDENSE_PROMPT must NOT be invoked."""
    from src import prompter
    src = tmp_path / "video.txt"
    src.write_text(_HEADER + "\n\nshort body.", encoding="utf-8")
    fake = _fake_llm("condensed body.")  # only 1 response — extra calls would StopIteration
    mocker.patch.object(prompter, "make_llm", return_value=fake)
    out = prompter.build_prompt_ready(src)
    assert "condensed body." in out.read_text(encoding="utf-8")


def test_build_prompt_ready_multiple_chunks_calls_merge(tmp_path, mocker, isolate_digest_dir):
    from src import prompter
    src = tmp_path / "video.txt"
    # Force >1 chunks. Use long sentences with rare boundaries to ensure split.
    long_body = ("This is a relatively long sentence about a topic. " * 200)  # ~10000 chars
...
def test_build_prompt_ready_no_header_uses_stem_as_title(tmp_path, mocker, isolate_digest_dir):
    from src import prompter
...
def test_build_prompt_ready_emits_lifecycle_events(tmp_path, mocker, isolate_digest_dir):
    from src import prompter
    src = tmp_path / "video.txt"
    src.write_text(_HEADER + "\n\nshort body.", encoding="utf-8")
    mocker.patch.object(prompter, "make_llm", return_value=_fake_llm("condensed."))
...
def test_build_prompt_ready_output_filename_matches_input(tmp_path, mocker, isolate_digest_dir):
    from src import prompter
