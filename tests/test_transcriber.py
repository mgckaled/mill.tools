...
# ── VAD time-saved reporting ─────────────────────────────────────────────────


@pytest.mark.unit
def test_transcribe_reports_vad_removed_silence(tmp_path, mocker, caplog):
    import logging as _logging

    _patch_whisper(
        mocker,
        [_Seg(0.0, 3.0, "hi")],
        info=_Info(duration=600.0, duration_after_vad=540.0),  # 60s removed (10%)
    )
    events: list[tuple[str, str, dict]] = []
    with caplog.at_level(_logging.INFO, logger="root"):
        _call_transcribe(tmp_path, on_event=lambda t, s, p: events.append((t, s, p)))

    vad = next(p for t, _, p in events if t == "vad_filtered")
    assert vad["removed"] == pytest.approx(60.0)
    assert vad["duration_after_vad"] == pytest.approx(540.0)
    assert any(
        "VAD removed" in r.message and "10%" in r.message for r in caplog.records
    )


@pytest.mark.unit
def test_transcribe_no_vad_line_when_attr_absent(tmp_path, mocker):
    # Default _Info has duration_after_vad=None → no event, no log line.
    _patch_whisper(mocker, [_Seg(0.0, 3.0, "hi")])
    events: list[tuple[str, str, dict]] = []
    _call_transcribe(tmp_path, on_event=lambda t, s, p: events.append((t, s, p)))
    assert not any(t == "vad_filtered" for t, _, _ in events)
...
@pytest.mark.unit
def test_transcribe_subtitle_formats_with_zero_segments_skips_write(
