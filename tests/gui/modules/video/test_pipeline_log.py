...
def test_resolve_stage_label_op_error():
    ...
    assert (
        resolve_stage_label(_make_event("video_op_error", {}))
...
def test_resolve_messages_op_start_uses_verb_and_item_name():
    ...
    ev = _make_event(
        "video_op_start", {"operation": "compress", "item_name": "movie.mp4"}
    )
...
def test_resolve_messages_op_error_includes_item_and_message():
    ...
    ev = _make_event(
        "video_op_error", {"item_name": "bad.mp4", "message": "codec not found"}
    )
    lines = resolve_messages(ev)
...
def test_resolve_messages_task_done_counts_outputs():
    ...
    lines = resolve_messages(ev)
    assert any("3 arquivo" in line for line in lines)
...
def test_fmt_subtitle_detail_soft_vs_hard():
    from src.gui.modules.video.pipeline_log import fmt_subtitle_detail

    soft = fmt_subtitle_detail("subs.srt", "soft")
    hard = fmt_subtitle_detail("subs.srt", "hard")
    assert "subs.srt" in soft
    assert "embutida" in soft
    assert "queimada" in hard
