...
_ALL_OPS = [
    "download", "convert", "resize", "crop", "rotate", "watermark",
    "border", "adjust", "filter", "favicon", "contact_sheet",
    "remove_bg", "describe",
]
...
def test_resolve_stage_label_queue_progress_includes_position_and_name():
    ...
    ev = _make_event("queue_progress", {
        "current_item": 3, "total_items": 7, "item_name": "photo.jpg",
    })
    label = resolve_stage_label(ev)
    assert "3/7" in label
...
def test_resolve_stage_label_op_error():
    from src.gui.modules.image.pipeline_log import resolve_stage_label
    assert resolve_stage_label(_make_event("image_op_error", {})) == "Erro — continuando fila..."
...
def test_resolve_messages_op_start_uses_verb_and_item_name():
    ...
    ev = _make_event("image_op_start", {"operation": "resize", "item_name": "photo.jpg"})
...
def test_resolve_messages_op_error_includes_item_and_message():
    from src.gui.modules.image.pipeline_log import resolve_messages
    ev = _make_event("image_op_error", {"item_name": "bad.png", "message": "cannot decode"})
    lines = resolve_messages(ev)
...
def test_resolve_messages_task_done_with_failed_count():
    from src.gui.modules.image.pipeline_log import resolve_messages
    ev = _make_event("task_done", {"output_paths": ["a.jpg", "b.jpg"], "failed_count": 1})
    lines = resolve_messages(ev)
    assert any("2 arquivo" in l for l in lines)
    assert any("1" in l and "erro" in l for l in lines)
...
def test_fmt_resize_detail_scale_pct_mode():
    ...
    out = fmt_resize_detail(
        "scale_pct", w_in=1920, h_in=1080,
        w_out=None, h_out=None, scale_pct=50.0,
    )
...
def test_fmt_resize_detail_contain_mode():
    from src.gui.modules.image.pipeline_log import fmt_resize_detail
    out = fmt_resize_detail(
        "contain", w_in=1920, h_in=1080,
        w_out=800, h_out=600, scale_pct=100.0,
    )
    assert "Caber" in out
    assert "L=800" in out
    assert "A=600" in out
